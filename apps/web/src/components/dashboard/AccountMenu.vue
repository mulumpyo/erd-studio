...
<template>
  ...
  <DropdownMenuRoot>
    <DropdownMenuTrigger
      class="hidden sm:flex h-9 items-center gap-2 rounded-xl px-2.5 text-[15px] font-semibold tracking-[-0.02em] transition-colors hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
    >
      <div
    ...
    <DropdownMenuPortal>
      <DropdownMenuContent
        align="end"
        :side-offset="6"
        class="z-50 min-w-[240px] overflow-hidden rounded-2xl border-0 bg-card p-1.5 shadow-[0_8px_32px_rgb(25_31_40_/_0.14)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95"
      >
  ...
  <DialogRoot :open="sheetOpen" @update:open="sheetOpen = $event">
    <DialogPortal>
      <DialogOverlay
      ...
      />
      <DialogContent
        class="fixed inset-x-0 bottom-0 z-50 rounded-t-2xl border-0 bg-card p-2 pb-safe shadow-[0_-4px_32px_rgb(25_31_40_/_0.18)] data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:slide-out-to-bottom data-[state=open]:slide-in-from-bottom focus:outline-none"
      >
        <!-- 드래그 핸들 -->
